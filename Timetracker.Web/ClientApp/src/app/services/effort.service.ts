...
import {
  IProject,
  IWeeklyEffort,
  IEffortUpsertRequest
} from "../models/effort.model";
@Injectable()
export class EffortService {
  ...
  getAllProjects(): Observable<IProject[]> {
    const projects$ = this.httpClient
      .get<IProject[]>(
        `${environment.endpoints.apiBaseUri}${appConstants.getProjectsEndPoint}`
      )
      .pipe(share());
    return projects$;
  }
  saveProject(project: IProject): Observable<{ id: number }> {
    return this.httpClient.post<{ id }>(
      `${environment.endpoints.apiBaseUri}${appConstants.getProjectsEndPoint}`,
      {
        ...project
      }
  ...
  saveEfforts(weeklyEffort: IWeeklyEffort, userName: string): Observable<any> {
    ...
    for (var i = 0; i < weeklyEffort.efforts.length; i++) {
      if (
        weeklyEffort.efforts[i].project.id > 0 &&
        weeklyEffort.efforts[i].effortPercent &&
        weeklyEffort.efforts[i].effortPercent > 0
      ) {
        if (
          weeklyEffort.efforts[i].id == 0 &&
          weeklyEffort.efforts[i].isDeleted == true
        ) {
    ...
    const userEfforts$ = this.httpClient.post<any>(
      `${environment.endpoints.apiBaseUri}${
